...
function normalize_status($s)
{
  ...
  if ($s === '') return 'pending';
  $s = strtolower($s);
  // replace multiple hyphens/underscores with spaces
  $s = preg_replace('/[-_]+/', ' ', $s);
  $s = preg_replace('/\s+/', ' ', $s);
  return $s;
}
...
<body>
  <div class="container py-5 mt-4">
    ...
    <div class="filter-container d-flex flex-wrap gap-3 justify-content-between align-items-center mb-3">
      <div>
        <label for="statusFilter" class="me-2">Status</label>
        <select id="statusFilter" class="form-select d-inline-block" style="width:200px;">
      ...
      <div>
        ...
        <select id="dateSort" class="form-select d-inline-block" style="width:200px;">
    ...
    <div id="ordersContainer">
      ...
      <?php else: ?>
        <?php foreach ($orders as $order):
          ...
          $total = (float)$order['price'] * max(1, (int)($order['quantity'] ?? 1));
        ?>
          <div class="order-item" data-status="<?= htmlspecialchars($normStatus) ?>" data-date="<?= (int) strtotime($order['order_date']) ?>">
            <div class="row g-5">
              <div class="col-auto">
                <a href="product-details.php?id=<?= (int)$order['product_id'] ?>&slug=<?= urlencode($order['product_title']) ?>">
                  <img src="admin/uploads/products/<?= htmlspecialchars($order['product_image'] ?? 'no-image.png') ?>" alt="" class="product-img">
                </a>
              </div>

              <div class="col">
                <a href="product-details.php?id=<?= (int)$order['product_id'] ?>&slug=<?= urlencode($order['product_title']) ?>" class="product-title text-decoration-none text-dark">
                  <?= htmlspecialchars($order['product_title']) ?>
                </a>

                <div class="mt-2">
                  <div class="pri-odr-sts">Price / unit: ₹<?= number_format((float)$order['price'], 2) ?></div>
                  <div class="pri-odr-sts">Quantity: <?= (int)($order['quantity'] ?? 1) ?></div>
                  <div class="pri-odr-sts">Size: <?= htmlspecialchars($order['size'] ?? '') ?></div>
                  <div class="pri-odr-sts"><strong>Total:</strong> ₹<?= number_format($total, 2) ?></div>
